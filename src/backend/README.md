...
`detections.csv` と各期間の分析CSVは実行時生成物です。Git管理はせず、存在しない場合は必要なヘッダー付きで自動生成されます。動作確認用データには `detections_sample.csv` を使用します。
...
2.APIが動くか確認
仮想環境で
uvicorn backend.api:app --app-dir src --reload
表示されたリンクをブラウザで開く
